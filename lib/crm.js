...
export function normalizeLead(payload, request) {
  ...
  const zipCode = cleanText(payload.zipCode || payload.zip, 80);
  ...
  if (!/^[\p{L}\p{N}][\p{L}\p{N}\s.,'’()\-\/]{1,79}$/u.test(zipCode)) throw new Error('Please enter the delivery country and postal code or city.');
